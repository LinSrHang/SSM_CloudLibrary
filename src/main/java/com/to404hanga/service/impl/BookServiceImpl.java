...
import com.to404hanga.mapper.BookMapper;
import com.to404hanga.service.BookService;
...
public class BookServiceImpl implements BookService {
    ...
    public Integer borrowBook(Book book) {
        // 设置借阅状态: 0可借阅 1借阅中 2归还中 3已经下架
        book.setStatus("1");
        // 设置借阅时间
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        String borrowTime = format.format(new Date());

        // 前端校验不可靠，后端校验借阅时间是否不大于归还时间
        if (borrowTime.compareTo(book.getReturnTime()) > 0) {
            return -1;
        }
